...
//       `${host}/api/appointment/reschedule/${selectedBookingId}`,
//       {
...
//     if (response.data.success) {
//       // Update the local state with the new appointment details
//       setBookings(prevBookings =>
//         prevBookings.map(booking =>
//           booking.id === selectedBookingId
//             ? {
//                 ...booking,
//                 date: new Date(rescheduleDate).getDate(),
//                 day: new Date(rescheduleDate).toLocaleDateString("en-US", { weekday: "short" }),
//                 month: new Date(rescheduleDate).toLocaleDateString("en-US", { month: "short" }),
//                 year: new Date(rescheduleDate).getFullYear(),
//                 fullDate: new Date(rescheduleDate),
//                 time: rescheduleTime,
//               }
//             : booking
//         )
...
//   // Set initial values from the existing appointment
//   const booking = bookings.find(b => b.id === bookingId);
//   if (booking) {
//     setRescheduleDate(booking.fullDate.toISOString().split('T')[0]);
...
//                           >
//                             <Clock className="w-4 h-4 mr-2" />Request Reschedule 
//                           </button>
...
//       {/* Reschedule Dialog */}
// <Dialog open={rescheduleDialogOpen} onClose={() => setRescheduleDialogOpen(false)} className="relative z-10">
//   <DialogBackdrop
//     transition
//     className="fixed inset-0 bg-gray-500/75 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"
//   />
...
import { Dialog, DialogBackdrop, DialogPanel, DialogTitle } from '@headlessui/react';
import { ExclamationTriangleIcon } from '@heroicons/react/24/outline';
import RescheduleAppointment from './Reschedule.jsx'; 
...
export default function Bookings() {
  ...
  const handleRescheduleSuccess = (updatedAppointment) => {
    ...
    setBookings(prevBookings =>
      prevBookings.map(booking =>
        booking.id === updatedAppointment._id
          ? {
              ...booking,
              date: new Date(updatedAppointment.date).getDate(),
              day: new Date(updatedAppointment.date).toLocaleDateString("en-US", { weekday: "short" }),
              month: new Date(updatedAppointment.date).toLocaleDateString("en-US", { month: "short" }),
              year: new Date(updatedAppointment.date).getFullYear(),
              fullDate: new Date(updatedAppointment.date),
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 lg:p-8">
      ...
      {/* Reschedule Dialog - Using your RescheduleAppointment component */}
      <Dialog open={rescheduleDialogOpen} onClose={() => setRescheduleDialogOpen(false)} className="relative z-10">
        <DialogBackdrop
          transition
          className="fixed inset-0 bg-gray-500/75 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"
            ...
            >
              {selectedBookingDetails && (
                <RescheduleAppointment
                  appointmentId={selectedBookingDetails.id}
                  currentDate={selectedBookingDetails.currentDate}
                  currentTime={selectedBookingDetails.currentTime}
                  doctorId={selectedBookingDetails.doctorId}
